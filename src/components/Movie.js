...
const Movie = (props) => {
  ...
  const handleFavorites = () => {
    !(favorites.includes(movie)) && dispatch(addFavorite(movie));
    push('/movies');
  }
  ...
  return (
    <div className="bg-white rounded-md shadow flex-1">
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        ...
        <button className="myButton bg-blue-600 hover:bg-blue-500" onClick={handleFavorites}>Favorilere ekle</button>
      </div>
    </div>
  );
}
